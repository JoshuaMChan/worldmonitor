...
import { applyZhLiveChannelTitleGlossary } from '@/utils/localized-fallback-titles';
...
function getLocalizedChannelName(channel: LiveChannel): string {
  ...
  if (!getLocale().toLowerCase().startsWith('zh')) return channel.name;
  return applyZhLiveChannelTitleGlossary(channel.name);
}
